...
function resolveGoogleNewsUrl(googleUrl: string, itemXml: string): string {
  // Try to extract original URL from <source url="..."> or description href
  const sourceUrl = extractAttr(itemXml, "source", "url");
  if (sourceUrl && !sourceUrl.includes("google.com")) return sourceUrl;

  // Extract from description anchor
  const descMatch = itemXml.match(/href="(https?:\/\/(?!news\.google\.com)[^"]+)"/);
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Google News RSS — real Google-powered news & web results, no API key required
    ...
    const results: WebSearchResult[] = itemBlocks.slice(0, 12).map((item) => {
      ...
      const resolvedUrl = resolveGoogleNewsUrl(rawLink, item);
